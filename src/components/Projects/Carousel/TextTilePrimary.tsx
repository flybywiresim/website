...
/**
 * Text Tile (Variant 1)
 * Dark gradient background with diagonal watermark text.
 * 1:1 square on the left, description revealed on the right when active.
 */
const TextTilePrimary = ({
    title,
    watermarkText = 'FLYBYWIRE',
    description = '',
    className,
    isActive = false,
}: TextTilePrimaryProps) => (
    <div
        className={twMerge(
    ...
    >
        ...
        <div className="relative flex h-full w-full items-end">
            ...
            <div className="flex h-full min-w-[18.6667rem]">
                {description && (
                    <span className="block p-4 md:p-6 text-right text-white">
                        {description}
                    </span>
                )}
            </div>
...
export default TextTilePrimary;
