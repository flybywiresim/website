...
VRAM exhaustion is a common occurance in MSFS2024 due to the increased VRAM requirements of the sim. As the A380 is an aircraft that is physically much larger than any other aircraft, this results in requiring much more texture space to wrap around the model with a similar texel density. Consider if you had to gift wrap an A380, you would need more rolls of wrapping paper compared to wrapping an A320. This is the same as texture space where larger aircraft simply need larger amount of textures, which leads to more memory consumed. This can be a big problem for GPUs with limited VRAM such as 8GB cards. As such, a lot of our development was initially done on a AMD Ryzen 5 5600X CPU with a RTX 3070 8GB GPU to ensure the aircraft performs decently on a limited VRAM situation, using reasonable sim settings to balance between visual fidelty and simulator performance.
